...
vector< vector<float> > normalize(vector< vector <float> > grid) {
    ...
    // Normalize the grid by dividing each element by the total sum
    ...
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            newGrid[i][j] = grid[i][j] / total_sum;
        }
    }
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
    ...
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            ...
            // Apply blur to the center cell
            newGrid[i][j] += center_prob * grid[i][j];

            // Apply blur to adjacent cells
            newGrid[(i + 1) % height][j] += adjacent_prob * grid[i][j];
            newGrid[(i - 1 + height) % height][j] += adjacent_prob * grid[i][j];
            newGrid[i][(j + 1) % width] += adjacent_prob * grid[i][j];
            newGrid[i][(j - 1 + width) % width] += adjacent_prob * grid[i][j];

            // Apply blur to corner cells
            newGrid[(i + 1) % height][(j + 1) % width] += corner_prob * grid[i][j];
            newGrid[(i - 1 + height) % height][(j + 1) % width] += corner_prob * grid[i][j];
            newGrid[(i + 1) % height][(j - 1 + width) % width] += corner_prob * grid[i][j];
            newGrid[(i - 1 + height) % height][(j - 1 + width) % width] += corner_prob * grid[i][j];
        }
    }
