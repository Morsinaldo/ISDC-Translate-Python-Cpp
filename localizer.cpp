...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
    ...
    // Calculate the initial belief for each cell
    float initial_belief = 1.0 / total_cells;

    // Initialize the new grid with uniform beliefs
    ...
    float blurring)
{
    ...
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            int new_i = (i + dy + height) % height;  // Implementing cyclical grid
            int new_j = (j + dx + width) % width;

            // Update the new grid with the beliefs after motion
    ...
    float p_miss)
{
    ...
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            // Update belief based on whether the sensed color matches the grid color
            float sense_prob = (grid[i][j] == color) ? p_hit : p_miss;
            newGrid[i][j] = beliefs[i][j] * sense_prob;
        }
    }
